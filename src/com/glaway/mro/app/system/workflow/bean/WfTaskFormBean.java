...
public class WfTaskFormBean extends DataBean {
	...
	public int dialogok() throws IOException, MroException {
		try {
			...
			formProperties.put(WFConstant.APPROVED,
					getJpo().getString("ACTIONID"));
			...
			if (actAppInfo != null) {
				...
				WfControlUtil.updateHistoryLog(processInstanceId, taskId,
						getJpo().getString("MEMO"), userId);
			}
			...
			if (StringUtil.isEqualIgnoreCase(appName, "FAILUREORD")) {
				...
				if (transPlan != null) {
					...
					// 当前办事处主任负责的改造分布
					IJpoSet transDistSet = MroServer.getMroServer()
							.getSysJpoSet(
									"TRANSDIST",
									"transplannum='"
											+ transPlan
	...
	public synchronized String[][] getDataList(String dataAttribute)
		...
		if (dataAttribute.equals("ACTIONID")) {
			...
			try {
				...
				Map<String, Object> properties = WfControlUtil
						.findTaskForm(taskId);
				if (properties.get(WFConstant.APPROVED) != null) {
					Map<String, String> values = (Map<String, String>) properties
							.get(WFConstant.APPROVED);
					if (values != null) {
						raidos = new String[values.size()][2];
						for (Entry<String, String> enumEntry : values
								.entrySet()) {
						...
						getPage().getControlByXmlId("startWorkflowActionId")
								.setShowOrHideFlag(Property.HIDE_CTRL);
					}
				} else {
					// 隐藏单选按钮标签
					// getPage().getControlByXmlId("startWorkflowActionId").hide();
	...
	/**
	 * 故障工单中增加下一节点任务执行人记录 修改人 杨毅 2018-09-26 <功能描述>
	 * 
	 ...
	 * 
	 */
	private void setFailureActperson(String processInstanceId)
