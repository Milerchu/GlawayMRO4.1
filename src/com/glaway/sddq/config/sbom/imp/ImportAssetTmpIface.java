package com.glaway.sddq.config.sbom.imp;
...
import com.glaway.mro.app.model.data.AssetModel;
 ...
 * @since  [产品/模块版本]
 */
public class ImportAssetTmpIface extends ImpObjectIface
{
    /**
     * 
     * <导入excel中的数据>
                        ...
                        try
                        {
                            ...
                            if ((cell != null) && (!ExcelUtil.getStringValue(cell).equals("")))
                            {
                                ...
                                for (String key : set)
                                {
                                    ...
                                    if (cell != null)
                                    {
                                        if (key != null && key.toUpperCase().equals("ASSETTMPLEVEL")
                                            && ExcelUtil.getStringValue(cell).equals("ASSET"))
                                        {
                                            ...
                                            IJpoSet jposet =
                                                MroServer.getMroServer().getSysJpoSet("ASSETTMP",
                                                    "ancestor = '" + ancestor + "' and ASSETTMPLEVEL = 'ASSET'");
                                            if (jposet != null && jposet.count() > 0)
                                            {
                                                String delAssetSqlString =
                                                    "delete from ASSETTMP where ancestor = '" + ancestor + "'";
                                                String delAssettreeSqlString =
                                                    "delete from assettmptree where assettmpnum in (select assettmpnum from assettmptree where ancestor='"
                                                        + ancestor + "')";
                                ...
                                if (!((String)acsvalues.get("PARENT")).equals("null"))
                                {
                                    ...
                                    if (!parent.equals(""))
                                    {
                                        ancestorMap.put((String)acsvalues.get("ASSETTMPNUM"), parent);
                                    }
                                    else
                                    {
                                        ancestorMap.put((String)acsvalues.get("ASSETTMPNUM"), null);
                                    }
                                }
                                else
                                {
                                ...
                                String insertAssetJz = getInsertSql(asAttrMap, acsvalues, "ASSETTMP");
                                System.out.println("---insert: " + i + "-" + insertAssetJz);
                                ancestorSql.add(insertAssetJz);
                                ancestorSql =
                                    getInsertAncestorSql("ASSETTMPTREE",
                                        ancestorSql,
                                        ancestorMap,
                                        (String)acsvalues.get("ASSETTMPNUM"),
                                        (String)acsvalues.get("ASSETTMPNUM"),
    ...
    public HashMap<String, String> getAssetInsertAttrMap()
    {
        ...
        attrMap.put("ASSETTMPID", "BIGINT");
        attrMap.put("ASSETTMPLEVEL", "UPPER");
        attrMap.put("ASSETTMPNUM", "UPPER");
    ...
    {
        acsvalues.put("ASSETTMPID", "ASSETTMPSEQ.NEXTVAL");
        acsvalues.put("ASSETTMPLEVEL", "null");
        acsvalues.put("ASSETTMPNUM", "null");
        ...
        HashMap<Object, Object> ancestorMap, String assetnum, String parent, String assetancestor, int i)
    {
        if (ancestorMap.containsKey(parent))
        {
            ...
            insertSql = insertSql + "ASSETTMPTREEID,ASSETTMPNUM,ANCESTOR,SITEID,ORGID,HIERARCHYLEVELS)";
            insertSql =
                insertSql + " values (ASSETTMPTREESEQ.NEXTVAL,'" + assetnum + "','" + parent + "','ELEC','CRRC'," + i
                    + ")";
        ...
        else
        {
            if ((parent.equals(assetancestor)))
            {
                ...
                insertSql = insertSql + "ASSETTMPTREEID,ASSETTMPNUM,ANCESTOR,SITEID,ORGID,HIERARCHYLEVELS)";
                insertSql =
                    insertSql + " values (ASSETTMPTREESEQ.NEXTVAL,'" + assetnum + "','" + parent + "','ELEC','CRRC',"
                        + i + ")";
                sqlList.add(insertSql);
            }
        }
     ...
     *
     */
    public void deleteExistData(String assettmpnum)
        throws MroException
    {
        /* 删除结构树数据*/
        ...
        jpoSet.reset();
        AssetModel jpo = (AssetModel)jpoSet.getJpo(0);
        
        jpo.deleteAssetTmpAncestor(jpo);
        jpoSet.save();
        
        /*删除主表数据*/
        jpoSet.setUserWhere("ancestor='" + assettmpnum + "'");
        jpoSet.reset();
        jpoSet.deleteAll();
        jpoSet.save();
    }
