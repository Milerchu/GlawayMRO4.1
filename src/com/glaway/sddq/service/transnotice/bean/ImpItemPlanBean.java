 ...
 * @since [产品/模块版本]
 */
public class ImpItemPlanBean extends AddDoclinkBean {
	...
	public void impData(File tmpFile, String filename) throws Exception {
		...
		if (sheet == null) {
			...
			impExcelnr(workbook, "MATERIALLIST", 1);
		}
	}
	...
	private void impExcelnr(Workbook workbook, String table, int hang)
		...
		for (int zs = 0; zs < numberOfSheets; zs++) {
			...
			for (int i = hang; (i < rsRows) && (lastRowNum > 0); ++i) {// 从第几行开始
				...
				String noticenum = this.page.getAppBean().getJpo()
						.getString("TRANSNOTICENUM");// 通知单编号
				materiallist.setValue("TRANSNOTICENUM", noticenum);
