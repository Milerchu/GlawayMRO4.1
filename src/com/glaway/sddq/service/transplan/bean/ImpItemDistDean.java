...
/**
 * 
 * 改造计划-物料计划-物料分发表导入databean
 * 
 * @author zhuhao
 * @version [版本号, 2018年10月16日]
 * @since [产品/模块版本]
 */
public class ImpItemDistDean extends AddDoclinkBean {
	...
	public void impData(File tmpFile, String filename) throws Exception {
		...
		if (sheet == null) {
			...
			impExcelnr(workbook, "TRANSPLANLOC", 1);
		}
	}
	...
	private void impExcelnr(Workbook workbook, String table, int hang)
		...
		IJpoSet materiallistSet = jpo.getJpoSet("$MATERIALLIST", table, "1=2");
		IJpoSet ijpoSet = jpo.getJpoSet("$IMPATTRIBUTE", "IMPATTRIBUTE",
				"ifacename='TPITEMPLANIMP'");
		...
		for (int zs = 0; zs < numberOfSheets; zs++) {
			...
			for (int i = hang; (i < rsRows) && (lastRowNum > 0); ++i) {// 从第几行开始
				IJpo materiallist = materiallistSet.addJpo();
				...
				for (int j = 0; j < ijpoSet.count(); j++) {
					try {
						...
						if ("QTY".equals(attributename)) {
							IJpo mplan = this.getDialog().getCreator()
									.getDataBean().getParent().getJpo();

							double distqty = mplan.getDouble("DISTEDQTY");// 已分发数量
							double mcount = mplan.getDouble("MATERIALCOUNT");// 计划数量
							double undistqty = mcount - distqty;// 未分发数量
							double qty = getDoubleByCell(cell);// 当前分发数量
							if (undistqty - qty <= 0) {
								throw new MroException("transplan",
										"outofundist");
							}
							mplan.setValue("DISTEDQTY", distqty + qty);// 设置已分发数量

						}

						materiallist.setValue(
								attributename,// 给字段赋值并区分类型
								getStringByCell(cell).trim(),
