 ...
 * @since  [产品/模块版本]
 */
public class WdrClose implements ActionCustomClass {
	...
	public void executeCustomAction(IJpo jpo, String parameter)
		...
		if (jpo != null) {
			IJpoSet wdrlilneset=jpo.getJpoSet("wdrline");
			String ERPLOC=jpo.getJpoSet("location").getJpo(0).getString("ERPLOC");
			if(ERPLOC.equalsIgnoreCase("1020")){
				...
				if (retu.equalsIgnoreCase("S")) {
					// 调用出库方法
					wdrout(jpo);
					jpo.setValue("status", "处置完成",GWConstant.P_NOCHECK_NOACTION_NOVALIDAT);
					inventoryqty(jpo,wdrlilneset);

				} else {
					throw new MroException("错误", "处置接口错误:" + retu);
				}
			}else{
				wdrout(jpo);
				jpo.setValue("status", "处置完成",GWConstant.P_NOCHECK_NOACTION_NOVALIDAT);
	...
	// return retu;
		String num = "";
		String DEALTYPE = jpo.getString("DEALTYPE");//处置类型
		String mrowerks = jpo.getJpoSet("LOCATION").getJpo()
				.getString("ERPLOC");
		...
		try {
			...
			if(DEALTYPE.equalsIgnoreCase("报废")){
				movetype.setChar3("551");// 参数--移动类型201
			}
			

			person.setChar10(CREATEBY);// 参数--点收入
			werks.setChar4(mrowerks);// 参数--工厂
			budat.setDate(CREATEDATES);// 参数--过账日期
			IJpoSet wdrlineset = jpo.getJpoSet("wdrline");
			TableOfBapi2017GmItemCreate table = new TableOfBapi2017GmItemCreate();
			JSONArray jArray = new JSONArray();
			for (int i = 0; i < wdrlineset.count(); i++) {
				IJpo wdrline = wdrlineset.getJpo(i);
				String itemnum = wdrline.getString("itemnum");// 物料编码

				String qty = wdrline.getString("SCRAPQTY");// --数量
				String WORKORDERNUM = jpo.getString("WORKORDERNUM");// 工作令号
				String unit = wdrline.getJpoSet("item").getJpo()
						.getString("ORDERUNIT");// 计量单位
				String COSTCENTER = jpo.getString("COSTCENTER");// 成本中心号
				String lotnum = "";
				String location = "";
				location = jpo.getJpoSet("location").getJpo()
			...
			num = IFUtil.addIfHistory(IFUtil.ERP_MRO_TOEROCKIF_201or551,
			...
			IFUtil.updateIfHistory(
					...
					IFUtil.FLAG_YES,
					"处置管理单号：" + jpo.getString("wdrnum") + ";处置类型："+DEALTYPE+";传递ERP"
							+ wdrlineset.count() + "条;");
			...
			IFUtil.updateIfHistory(num, IFUtil.STATUS_SUCCESS, IFUtil.FLAG_YES,
					"处置管理单号：" + jpo.getString("wdrnum") + ";处置类型："+DEALTYPE+";接收ERP回传;");
	 ...
	 * 
	 */
	public void wdrout(IJpo wdrJpo) throws MroException {// 出库
		try {
			// IJpo mprJpo = jpo;
			IJpoSet wdrlineset = wdrJpo.getJpoSet("wdrline");
			String location = wdrJpo.getString("location");
			if (!wdrlineset.isEmpty()) {
				for (int i = 0; i < wdrlineset.count(); i++) {
					IJpo wdrline = wdrlineset.getJpo(i);
					double qty = wdrline.getDouble("SCRAPQTY");// --数量
					String itemnum = wdrline.getString("itemnum");// --物料编码
					...
					if (!inventoryset.isEmpty()) {
						...
						inventoryset.save();
						wdroutinventrans(wdrline);
						String type = ItemUtil.getItemInfo(itemnum);
						if (ItemUtil.SQN_ITEM.equals(type)) {
							String assetnum=wdrline.getString("assetnum");
							String DEALTYPE =wdrJpo.getString("DEALTYPE");
							assetchange(assetnum,DEALTYPE);
						}
					}
				}
			}
	...
	public void wdroutinventrans(IJpo wdrline) { // 出库
		try {
			IJpoSet INVTRANSset = MroServer.getMroServer().getJpoSet(
					"INVTRANS", MroServer.getMroServer().getSystemUserServer());
				java.util.Date transdate = MroServer.getMroServer().getDate();// --交易时间
				double qty = wdrline.getDouble("SCRAPQTY");// --数量
				String location = wdrline.getString("location");// --库房
				String itemnum = wdrline.getString("itemnum");// --物料编码
				String wdrnum = wdrline.getString("wdrnum");// --领料单编号
				String siteid = wdrline.getString("siteid");// --地点
				String orgid = wdrline.getString("orgid");// --组织
				String assetnum = wdrline.getString("assetnum");// --资产编号
				String sqn = wdrline.getString("sqn");// --序列号
				String lotnum = wdrline.getString("lotnum");// --批次号
				String DEALTYPE = wdrline.getString("DEALTYPE");// --批次号

				IJpo INVTRANS = INVTRANSset.addJpo();
				INVTRANS.setValue("TRANSTYPE", DEALTYPE+"出库",
				...
				INVTRANS.setValue("mprnum", wdrnum,
						GWConstant.P_NOCHECK_NOACTION_NOVALIDAT);// --处置编号
	...
	public void assetchange(String assetnum,String DEALTYPE) { //序列号去库房信息
		try {
			IJpoSet assetset = MroServer.getMroServer().getJpoSet(
					"asset", MroServer.getMroServer().getSystemUserServer());
			...
			if(!assetset.isEmpty()){
				...
				if(DEALTYPE.equalsIgnoreCase("报废")){
					asset.setValue("status", "报废",GWConstant.P_NOCHECK_NOACTION_NOVALIDAT);
				}
				assetset.save();
			}
		} catch (MroException e) {
	 ...
	 *
	 */
	public void inventoryqty(IJpo wdr,IJpoSet wdrlineset) throws MroException {// 出库
		if(wdr!=null){
			String LOCATION=wdr.getString("LOCATION");
			if(!wdrlineset.isEmpty()){
				for(int i=0;i<wdrlineset.count();i++){
					IJpo wdrline=wdrlineset.getJpo(i);
					String itemnum=wdrline.getString("itemnum");
