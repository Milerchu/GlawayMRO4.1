...
public class FldIssuestoreroom extends JpoField {
	...
	public IJpoSet getList() throws MroException {
		...
		String RECEIVESTOREROOM = this.getJpo().getParent()
				.getString("RECEIVESTOREROOM");
		setListObject("LOCATIONS");
		if (!RECEIVESTOREROOM.isEmpty()) {
			String erploc = this.getJpo().getParent()
			...
			if (mrnum.isEmpty()) {
				...
				if (TRANSFERMOVETYPE.equalsIgnoreCase("中心到中心")) {
					String Sql = "location not in (select location from locations where ISSAPSTORE=1) and STOREROOMLEVEL in('中心库') and erploc='"
							+ erploc + "'";
					Sql=Sql+"and status='正常'";
					setListWhere(Sql);
				}
