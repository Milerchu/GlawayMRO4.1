 ...
 * @since  [产品/模块版本]
 */
public class SelectTransferlinetradeDataBean extends DataBean {
	/**
	 * 过滤装箱单信息
	 * @throws MroException
	 */
	 public void initialize()
			...
			if(transferlineset == null || transferlineset.count() == 0){
				...
				where="RECEIVESTOREROOM='"+ISSUESTOREROOM+"'  and not exists(select zxdlineid from transferline where zxdlineid is not null and zxdlineid=transferlinetradeid)";
			}
			if(transferlineset != null && transferlineset.count() >0){
				...
				if (Idstr != null)
		        {
					String where1="RECEIVESTOREROOM='"+ISSUESTOREROOM+"' and not exists(select zxdlineid from transferline where zxdlineid is not null and zxdlineid=transferlinetradeid)";
 ...
 */
	...
	public int execute() throws IOException, MroException {
		DataBean transferlineBean = this.page.getAppBean().getDataBean("1527057849697");
		...
		String ISSUEADDRESS =transfer.getString("ISSUEADDRESS");
		String RECEIVEADDRESS =transfer.getString("RECEIVEADDRESS");
		...
		if(!list.isEmpty()){
			for (IJpo iJpo : list) {
				...
				transferline.setValue("ISSUEADDRESS", ISSUEADDRESS,GWConstant.P_NOCHECK_NOACTION_NOVALIDAT);
				transferline.setValue("RECEIVEADDRESS", RECEIVEADDRESS,GWConstant.P_NOCHECK_NOACTION_NOVALIDAT);
