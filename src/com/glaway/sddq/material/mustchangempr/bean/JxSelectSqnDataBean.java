...
/**
 * 
 * <检修领料单选择序列号弹出框DataBean>
 * 
 * @author public2795
 * @version [版本号, 2019-1-8]
 * @since [产品/模块版本]
 */
public class JxSelectSqnDataBean extends DataBean {
	/**
	 * 过滤序列号数据
	 * 
	 * @throws MroException
		...
		if (!StringUtil.isStrEmpty(MPRSTOREROOM)) {
			if (!StringUtil.isStrEmpty(where)) {
				where = where
						+ " and  location='"
						+ MPRSTOREROOM
						+ "' and assetnum not in(select assetnum from MUSTCHANGEMPR where itemnum='"
						+ itemnum
						+ "' and mprnum in (select mprnum from mpr where status!='关闭' and mprtype='JX')) and itemnum='"
						+ itemnum + "'";
			} else {
				where = " location='"
						+ MPRSTOREROOM
