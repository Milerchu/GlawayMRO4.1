...
public class FldJcJobTestRecord extends JpoField {
        ...
        throws MroException
    {
        ...
        if (getJpo() != null)
        {
            String schemenum = getJpo().getString("SCHEMENUM");
            indexColSet =
                getUserServer().getJpoSet("JCJOBTESTRECORD",
                    "schemenum='" + StringUtil.getSafeSqlStr(schemenum) + "' and " + getJpo().getIdColName() + "!="
                        + getJpo().getId());
            indexColSet.reset();
        }
        return indexColSet;
