 ...
 * @since  [产品/模块版本]
 */
public class ImpJcJobTestREecordIng extends AddDoclinkBean {
	...
	public void impData(File tmpFile, String filename) throws Exception {
		...
		if (sheet == null) {
			...
			impExcelnr(workbook, "JCJOBTESTRECORDING", 1);
		}
	}
	...
	private void impExcelnr(Workbook workbook, String table, int hang)
		...
		IJpoSet jcjobtestrecordingSet = jpo
				.getJpoSet("$JCJOBTESTRECORDING", table, "1=2");
		IJpoSet ijpoSet = jpo.getJpoSet("$IMPATTRIBUTE", "IMPATTRIBUTE",
				"ifacename='JCJOBTESTRECORDINIMP'");
		...
		for (int zs = 0; zs < numberOfSheets; zs++) {
			...
			for (int i = hang; (i < rsRows) && (lastRowNum > 0); ++i) {// 从第几行开始
				IJpo jcjobtestrecording = jcjobtestrecordingSet.addJpo();
				...
				for (int j = 0; j < ijpoSet.count(); j++) {
					try {
						...
						jcjobtestrecording.setValue(attributename,// 给字段赋值并区分类型
