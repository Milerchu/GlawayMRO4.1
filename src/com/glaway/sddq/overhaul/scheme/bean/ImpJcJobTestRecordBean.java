 ...
 * @since  [产品/模块版本]
 */
public class ImpJcJobTestRecordBean extends AddDoclinkBean {
	...
	public void impData(File tmpFile, String filename) throws Exception {
		...
		if (sheet == null) {
			...
			impExcelnr(workbook, "JCJOBTESTRECORD", 1);
		}
	}
	...
	private void impExcelnr(Workbook workbook, String table, int hang)
		...
		IJpoSet jcjobtestrecordSet = jpo
				.getJpoSet("$jcjobtestrecord", table, "1=2");
		IJpoSet ijpoSet = jpo.getJpoSet("$IMPATTRIBUTE", "IMPATTRIBUTE",
				"ifacename='JCJOBTESTRECORDIMP'");
		...
		for (int zs = 0; zs < numberOfSheets; zs++) {
			...
			for (int i = hang; (i < rsRows) && (lastRowNum > 0); ++i) {// 从第几行开始
				IJpo jcjobtestrecord = jcjobtestrecordSet.addJpo();
				...
				for (int j = 0; j < ijpoSet.count(); j++) {
					try {
						...
						Cell schemenum = row.getCell(0);
						IJpoSet repairschemeSet = MroServer.getMroServer()
								.getSysJpoSet(
										"REPAIRSCHEME",
										"SCHEMENUM = '" + getStringByCell(schemenum)
												+ "'");
						if (repairschemeSet.count() < 1) {
							throw new AppException("repairscheme",
									"atypism");
						}
						
						String attributename = ijpo.getString("attributename");

						jcjobtestrecord.setValue(attributename,// 给字段赋值并区分类型
