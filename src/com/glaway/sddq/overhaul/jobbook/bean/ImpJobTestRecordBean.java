 ...
 * @since  [产品/模块版本]
 */
public class ImpJobTestRecordBean extends AddDoclinkBean {
	...
	public void impData(File tmpFile, String filename) throws Exception {
		...
		if (sheet == null) {
			...
			impExcelnr(workbook, "JOBTESTRECORD", 1);
		}
	}
	...
	private void impExcelnr(Workbook workbook, String table, int hang)
		...
		IJpoSet jobtestrecordSet = jpo
				.getJpoSet("$jobtestrecord", table, "1=2");
		IJpoSet ijpoSet = jpo.getJpoSet("$IMPATTRIBUTE", "IMPATTRIBUTE",
				"ifacename='JOBTESTRECORDIMP'");
		...
		for (int zs = 0; zs < numberOfSheets; zs++) {
			...
			for (int i = hang; (i < rsRows) && (lastRowNum > 0); ++i) {// 从第几行开始
				IJpo jobtestrecord = jobtestrecordSet.addJpo();
				...
				for (int j = 0; j < ijpoSet.count(); j++) {
					try {
						...
						jobtestrecord.setValue(attributename,// 给字段赋值并区分类型
...
//						}
						...
						e.printStackTrace();
						throw new MroException("", "导入失败"
								+ sheet.getSheetName() + "第" + (i+1) + "行" + ",第"
								+ (j+1) + "列 填写有误，请核查后重新导入(注意数据类型)");
					}
