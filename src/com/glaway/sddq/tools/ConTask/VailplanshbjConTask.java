...
public class VailplanshbjConTask extends BaseStatefulJob {
	public void execute() throws MroException {
		...
		IJpoSet transplanJpoSet = MroServer.getMroServer().getSysJpoSet(
				"TRANSPLAN");
		transplanJpoSet.setUserWhere("STATUS='待审核'and plantype='验证'");
		...
		if (transplanJpoSet != null && transplanJpoSet.count() > 0) {
			for (int i = 0; i < transplanJpoSet.count(); i++) {
				...
				String bgsy = "报警事由";
				String message = "验证计划制定已超期，请尽快处理";
				String news = "相关信息";
				String result = "";
				String[] object = null;
				if (plansetdate != null) {
