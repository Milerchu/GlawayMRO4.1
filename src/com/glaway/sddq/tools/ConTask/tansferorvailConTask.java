...
/**
 * 
 * 改造计划进度通知 (通报改造完成进度,每周五)18
 * 
 * @author zzx
 * @version [版本号, 2018-8-30]
 * @since [产品/模块版本]
 */
public class tansferorvailConTask extends BaseStatefulJob {
	@Override
			...
			for (int i = 0; i < transplanJpoSet.count(); i++) {
				...
				for (int index = 0; index < transdistJpoSet.count(); index++) {
					projectnum = transdistJpoSet.getJpo(index).getString(
							"PROJECTNUM");// 所属项目
					String abbreviation = transdistJpoSet.getJpo(index)
							.getString("SYS_DEPT.ABBREVIATION");// 办事处

					String station = transdistJpoSet.getJpo(index).getString(
							"STATION");// 配属站段

					String transcount = transdistJpoSet.getJpo(index)
							.getString("TRANSCOUNT");// 改造数量

					String surplus = transdistJpoSet.getJpo(index).getString(
							"SURPLUS");// 剩余数量

					String responsible = transdistJpoSet.getJpo(index)
							.getString("RESPONSIBLE");// 办事处主任
					// 取改造工单jposet
