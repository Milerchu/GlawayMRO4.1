...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			var kpiGauge = null;
			function drawGaugeChart(percent) {	
				...
				kpiGauge = new ibm.tivoli.mbs.dijit.KpiGauge({
					...
					hasParent: true,
					url: "",
					ranges: [{low:0, high:100, hover: null, color: '#e5e2e2'}],
					...
					indicators: [new ibm.tivoli.mbs.dijit.gauge.KpiArcRange({startvalue: 0,endvalue: 30,   url: "", width:20, offset:75, hover: "差", color: '#a63b3a', hideValue: true}),
								 ...
								 new ibm.tivoli.mbs.dijit.gauge.KpiArcRange({startvalue: 70,endvalue: 85,  url: "", width:20, offset:75, hover: "良好", color: '#b2ca2f', hideValue: true}),
								 new ibm.tivoli.mbs.dijit.gauge.KpiArcRange({startvalue: 85,endvalue: 100, url: "", width:20, offset:75, hover: "优", color: '#90d500', hideValue: true}),
								 new dojox.widget.gauge.AnalogNeedleIndicator({value: 90, color: '#575657', width: 6, length: 95, hover: "目标：90%", easing: 'dojo.fx.easing.linear', hideValue: true}),
								 new dojox.widget.gauge.AnalogArrowIndicator({value: percent, color: '#ffa500', width: 6, length: 95, hover: "当前值："+percent+"%", easing: 'dojo.fx.easing.bounceOut', hideValue: true})
					]
				}, 'kpiChart');
			...
			/* 加载首页罗表盘数据 */
			function loadHomeGaugeData() {
				$.ajax({ 
					url: CLIENT_PATH + "/portal/jsp/home/gaugeChartReq.jsp", 
					cache: false,
					dataType: "json",
					error: function() {
						//alert("dd");
					...
					success: function(dataResp){    		
				    	drawGaugeChart(dataResp.totalHealLedger.percent);//显示当前健康（活动）台帐所占比例图 
					}
				});
			}
			
			$(function() {
				loadHomeGaugeData(); //加载首页罗表盘数据
			});
		</script>
	</head>
