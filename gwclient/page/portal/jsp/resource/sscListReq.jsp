...
<%!
	int getResultSetCount(Statement st) throws SQLException  {
		String sql = "select count(*) as count from labor";
		int count = 0;
	...
	StringBuffer mboToJsonString(Statement st, int fromPos, int toPos, String[] props) throws SQLException {
		...
		String sql = "select * from "
					+ "("
							...
							+ "("
								+ "select t.*, lc.skilllevel, lc.craftname from "
								+ "( "
									+ "select la.*, p.displayname as personname, p.jobcode, p.title, p.department "   
									+ "from " 
									+ "labor la " 
									+ "left join " 
									+ "( "
										+ "select person.personid, person.displayname, person.jobcode, person.title, dept.description as department from "
										+ "person " 
										+ "left join " 
										+ "dept "
										+ "on person.department=dept.deptnum "		
									+ ") p "
									+ "on la.personid=p.personid " 
								+ ") t " 
								+ "left join " 
								+ "( "
									+ "select lc_i.*, cr.description as craftname " 
									+ "from LABORCRAFTRATE lc_i " 
									+ "left join " 
									+ "craft cr "
									+ "on lc_i.craft=cr.craft "
								+ ") lc "
								+ "on t.laborcode=lc.laborcode " 
								+ "where lc.defaultcraft=1 "
...
<%
	String[] props = {"laborcode", "personid", "status", "personname", "jobcode", "title", "department", "skilllevel", "craftname"};
