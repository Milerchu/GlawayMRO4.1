...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function getHoverStr(healData) {
				var hoverStr = "台帐总数：" + healData.total + "<br/>";
				hoverStr = "健康总数：" + healData.health + "<br/>";
				hoverStr = "健康比例：" + ((healData.total==0)? "无": (healData.percent + "%"));
				return hoverStr;
			}
			...
			function loadGaugeData() {
				$.ajax({ 
					url: CLIENT_PATH + "/portal/jsp/product/ledgerGaugeReq.jsp?type=" + type, 
					cache: false,
					dataType: "json",
					error: function() {
						//alert("dd");
					...
					success: function(dataResp){    		
						if(type == '-1') {
				    		drawGaugeChart(80, dataResp.healLedger.percent, getHoverStr(dataResp.healLedger));
				    	} else if(type == '1') {
							drawGaugeChart(90, dataResp.healLedger.percent, getHoverStr(dataResp.healLedger));
						} else {
							drawGaugeChart(70, dataResp.healLedger.percent, getHoverStr(dataResp.healLedger));
						}
					}
				});
			}
			...
			dojo.addOnLoad(function() {
				loadGaugeData(); //加载罗表盘数据
			});
		</script>
	</head>
	<body style="width:100%; height:100%; overflow: hidden;">
		 <div id="kpiChart" style="margin: 0 auto; border: 0 none; padding: 0;"></div>
	</body>
</html>
