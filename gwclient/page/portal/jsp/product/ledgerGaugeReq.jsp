...
<%!
	...
	*/
	StringBuffer mboToJsonString(int type) throws MXException, java.rmi.RemoteException, SQLException {
		...
		jsonStrTmp.append("{");
		jsonStrTmp.append("\"healLedger\": " + getHealPerc(st, type));
		jsonStrTmp.append("}");
	...
	String getHealData(Statement st, HealStr heal, String condStr) throws SQLException {
		...
		sql = "select count(*) as count "
			+ "from asset "
			+ "where assetlevel='ASSET' and status in (" +  RUN_LEDGER_STATUS + ") " //+ "where assetlevel='ASSET' and status='活动' "
			+ condStr;  
...
<%
	...
	pw.write(mboToJsonString(Integer.parseInt(type)).toString());
%>
