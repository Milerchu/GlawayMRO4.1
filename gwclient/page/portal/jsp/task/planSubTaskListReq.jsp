				...
				for(int relIndex = 0, REL_LENGTH = subRlas.length; relIndex < REL_LENGTH; relIndex++) {
					...
					if(subRlas[relIndex].type == 0) { //0:作为上层的属性
						if(subMsr != null && !(subMsr.isEmpty())) {
							...
							jsonStrTmp.append(", " + mboToJsonString(null, subRlas[relIndex].props, subRlas[relIndex].relation.toLowerCase() + "_"));
						}
					} else { //1:作为子层的属性
...
%>
<%
	String[] props = {"taskitemnum", "ordernum", "description", "legacynum", "schstarttime", "schcompletetime", "assetnum", "responsibility", "actcompletetime"};
	...
	/* 查询结果集 */
	MboSetRemote msr = MXServer.getMXServer().getMboSet("TASKITEM", MXServer.getMXServer().getUserInfo("MAXADMIN"));	
	String sqlWhere = "fieldtasknum='" + fieldtasknum + "' and siteid='" + siteid + "'"; 
	...
	subRlas[0].relation = "person";
	subRlas[0].props = new String[]{"displayname"};
	subRlas[0].type = 0;
