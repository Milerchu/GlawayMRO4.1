	...
	StringBuffer mboToJsonString(MboRemote mbo, String[] props, String prefix) throws MXException, java.rmi.RemoteException {
		...
		if(props != null && props.length > 0) { //但传入的指定属性非空时，以指定的属性列为封装对象
			...
			for(int j = 0, LENGTH = props.length; j < LENGTH; j++) {
				if(j == 0) {
					jsonStrTmp.append("\"" + prefix + "_" + props[j].toLowerCase() + "\":\"" + mbo.getString(props[j]) + "\"");
				} else {
					jsonStrTmp.append(", \"" + prefix + "_" + props[j].toLowerCase() + "\":\"" + mbo.getString(props[j]) + "\"");
				}
			}
			...
			while (e.hasMoreElements()) {
				...
				if(isFirst) {
					jsonStrTmp.append("\"" + prefix + "_" + columnName.toLowerCase() + "\":\"" + mbo.getString(columnName) + "\"");
					isFirst = false;
				} else {
					jsonStrTmp.append(", \"" + prefix + "_" + columnName.toLowerCase() + "\":\"" + mbo.getString(columnName) + "\"");
				}
				...
				for(int relIndex = 0, REL_LENGTH = subRlas.length; relIndex < REL_LENGTH; relIndex++) {
					...
					if(subRlas[relIndex].type == 0) { //0:作为上层的属性
						if(subMsr != null && !(subMsr.isEmpty())) {
							jsonStrTmp.append(", " + mboToJsonString(subMsr.getMbo(0), subRlas[relIndex].props, subRlas[relIndex].relation.toLowerCase()));
						}
					} else { //1:作为子层的属性
...
%>
<%
	String[] props = {"sencyclicnum", "content", "dutytype", "completetime", "siteid"};
	...
	/* 查询结果集 */
	MboSetRemote msr = MXServer.getMXServer().getMboSet("SENCYCLIC", MXServer.getMXServer().getUserInfo("MAXADMIN"));	
	String sqlWhere = "SENCYCLICNUM in (select SENCYCLICNUM from FTSENC where fieldtasknum='" + fieldtasknum + "' and siteid='" + siteid + "')"; 
	...
	/* 合并“客户”子层数据 */
	SubRla[] subRlas =  new SubRla[2];
	subRlas[0] = new SubRla();
	subRlas[0].relation = "secustinfo";
	subRlas[0].props = new String[]{"description", "armytype", "whicharmy", "custname"};
	subRlas[0].type = 0; 
	
	subRlas[1] = new SubRla();
	subRlas[1].relation = "sedept";
	subRlas[1].props = new String[]{"description"};
	subRlas[1].type = 0;
