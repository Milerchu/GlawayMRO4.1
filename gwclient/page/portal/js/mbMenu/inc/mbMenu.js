	...
	$.fn.extend({
		buildMbMenu: function(op,m,type,e){
			...
			$(this.voices).each(function(i){
				...
				if (op.options.hasImages && !isText && !isBoxmenu){
					...
					// 用背静方式取代原加载图片方式
					//imgPlace="<td class='img' style=\"background:url(" + op.options.iconPath+imgPath + ") center no-repeat;\"></td>";
				}

				var line="<table id='"+m+"_"+i+"' class='line"+(isTitle?" title":"")+"' cellspacing='0' cellpadding='0'><tr>"+imgPlace+"<td class='voice' nowrap></td></tr></table>";
		...
		removeMbMenu: function(op,fade){
			...
			if (op.rootMenu) {
				...
				/* 屏蔽此处，可将弹出的下拉选项固定在前段，鼠标移除热区后不会隐藏 */
