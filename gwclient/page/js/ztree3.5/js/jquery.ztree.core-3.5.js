...
(function($){
	...
	$.fn.zTree = {
		...
		init: function(obj, zSetting, zNodes, selectedNodeId) {
			...
			var zTreeTools = {
				...
				loadData: function(data) {
					...
					var pid = getTagInnerHtmlStr(data, "pid");
					var newNodes = getTagInnerHtmlStr(data, "subnodes");
					var parentNode = this.getNodeByParam("id", pid, null); 
					if(newNodes == null || newNodes == "" || newNodes == '[]') {
						setting.data.keep.parent = false;
						this.removeChildNodes(parentNode);
//						this.expandNode(parentNode, true, false, true);
					} else {
						this.removeChildNodes(parentNode);
						this.addNodes(parentNode, $.parseJSON(newNodes), false);
					}
				...
				collapseNode: function(data) {
					var pid = getTagInnerHtmlStr(data, "pid");
					var parentNode = this.getNodeByParam("id", pid, null); 
					this.expandNode(parentNode, false, false, true);
					this.selectNode(parentNode);
				},
				deleteNode: function(data) {
					var id = getTagInnerHtmlStr(data, "id");
					var cNode = this.getNodeByParam("id", id, null); 
					this.removeNode(cNode);
				},
				appendNode:function(data){
					var pid=getTagInnerHtmlStr(data, "pid");
					var newNode=getTagInnerHtmlStr(data, "newNode");
					var parentNode = this.getNodeByParam("id", pid, null);
					var isOpen = parentNode.open;
					if(isOpen === "true" || isOpen == true){
						newNode = eval("("+newNode+")");
						this.addNodes(parentNode, newNode,false);
						this.selectNode(newNode);
					}else{
						this.addNodes(parentNode, newNode);
						//parentNode.open = true;
						this.collapseNode(data);
						this.expandNode(parentNode,false);
						//parseClick(parentNode.treenodeid, true, 'expandNode');
					}
				},
				openURL: function(data){
					var url = getTagInnerHtmlStr(data, "url");
					window.open(url);
				},
				...
				reloadTree: function(data){
					var containerid = getTagInnerHtmlStr(data, "cid");
					var htmldata = getTagInnerHtmlStr(data, "htmldata");
					$("#"+containerid).replaceWith(htmldata);
				},
				rebuildTree:  function(data) {
					var settingTmp = this.setting;
					var treeId = getTagInnerHtmlStr(data, "treeId");
					var newNodes = getTagInnerHtmlStr(data, "nodes");
					var currNodeId = getTagInnerHtmlStr(data, "currNodeId");
					//将json数据变成数组
					var str = str2Json(newNodes);
					//获取session里面的数据库id
					var id = sessionStorage.getItem("treeNode.id");
					if(id == "undefined"){
						id = tempId;
					}else{
						tempId = id;
					...
					for(var i = 0;i<str.length;i++){
						//当id相等的时候，获取treenodeid
						if(str[i].id==id){
							treenodeid = str[i].treenodeid;
						}
					}
					this.destroy();
					ctrlMgrSys.replace(treeId, $.fn.zTree.init($("#" + treeId), settingTmp, str2Json(newNodes), currNodeId));
					//保存的时候，等树结构全部刷新的时候，点击保存前的节点，
					setTimeout(function(){
						debugger;
						var treenode ;
						//如果没有点，节点信息不存在了，就点击树节点的第一行。
						if(treenodeid==undefined){
							 treenode = {treenodeid : str[0].treenodeid};
							clickNode(treeId, treenode, 1);
						}else{
				...
				refreshTreeNode: function(data){
					var nodeids = getTagInnerHtmlStr(data, "nodeid");
					var arrayNodes = nodeids.split(",");
					for(i=0;i<arrayNodes.length;i++){
					   var node = this.getNodeByParam("id", arrayNodes[i], null);
					   this.updateNode(node);
					}
				},
				clickNode: function(data){
					var pid = getTagInnerHtmlStr(data, "pid");
					var treeNode = this.getNodeByParam("id", pid, null);
					this.selectNode(treeNode);
				},
				rightClickNode: function(data){
					var pid = getTagInnerHtmlStr(data, "pid");
					var x = getTagInnerHtmlStr(data, "x");
					var y = getTagInnerHtmlStr(data, "y");
					var menujson = getTagInnerHtmlStr(data, "menus");
					var needUpdate = getTagInnerHtmlStr(data, "update");
					var treeNode = this.getNodeByParam("id", pid, null);
					this.selectNode(treeNode);
					$("#"+treeNode.tId).rightmenu({
						menu:$("#"+this.setting.treeId+"_menu"),
						X:x,
						Y:y,
						mjson:menujson,
			...
			if(!undef(selectedNodeId)) {
				var selectedNode = zTreeTools.getNodeByParam("id", selectedNodeId);
				if(!undef(selectedNode)) {
					zTreeTools.selectNode(zTreeTools.getNodes()[0]);
				}
			}
			return zTreeTools;
		}
