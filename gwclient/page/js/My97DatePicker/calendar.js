...
My97DP.prototype = {init:function () {
	...
	$d.okI.value = $lang.okStr;//设置OK按钮的显示文字(button value="确定")
	...
	//if ($dp.el[$dp.elProp] != $) { //如果是新的日期值
		//取消错误样式残留
		var oldClass = $dp.el.className;
		if (oldClass) {
			oldClass = oldClass.replace(/WdateFmtErr/g, "");
		}
		$dp.el.className = oldClass;
		
		$dp.el[$dp.elProp] = $; //$dp.el为input的dom对象，$dp.elProp为value属性，$为新的日期值 input.value = $;
		//输入框日期变更事件处理。ff下，必须用getAttribute取dom中自定义的属性值
		parent.callCtrlMthd($dp.el.getAttribute("ctrlid"), "beforeDateChange", [$]); //local_library.js --> callCtrlMthd
	//}
	this.setRealValue();
	...
	if (!C) {
		_.a("<div style=\"float:right;cursor:pointer\" onclick=\"hide($d.qsDivSel);\">X&nbsp;</div>");
	}
	_.a("</td></tr>");
...
/* 点击图标后，初始化设置输入框里的原始值。如果输入框无值则设置当前时间值，但输入框内无变化 */
function sv(_, $) {
	if ($ == null) {
		$ = $dt[_];
	}
