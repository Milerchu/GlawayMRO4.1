...
draw2d.Draggable.prototype.construct = function(_6051) {
	...
	var _6053 = function() {
		...
		//console.log("parseClick_"+wfcanvasId+"_"+oThis.node.getNodeId());
		window.parent.parseClick(wfcanvasId,true,'selectNode',oThis.node.getPropDailogId(),oThis.node.getNodeId());
	};
	var _6056 = function() {
		//console.log("node_mousedown");
		oThis.node.getWorkflow().showMenu(null);
		if(wfcanvasReadonly==="false"){
...
draw2d.Figure.prototype.setPropDailogId = function(propDialogId) {
	this.propDialogId = propDialogId;
	if (this.html !== null) {
		this.html.propDialogId = propDialogId;
	}
};
draw2d.Figure.prototype.getPropDailogId = function() {
	return this.propDialogId;
};
draw2d.Figure.prototype.setNodeId = function(nodeId) {
	this.nodeId = nodeId;
	if (this.html !== null) {
		this.html.nodeId = nodeId;
	}
};
draw2d.Figure.prototype.getNodeId = function() {
	return this.nodeId;
};
...
draw2d.Figure.prototype.setWorkflow = function(_59c7) {
	if (this.draggable === null) {
		...
		this.keyDown = function(event) {
			//console.log("node_keydown");
			//console.log("oThis.wfcanvasReadonly="+wfcanvasReadonly)
			if(wfcanvasReadonly==="false"){
		...
		this.tmpDragend = function(_59cd) {
			oThis.onDragend();
			if(oThis.ox==-1 && oThis.oy==-1){
				oThis.ox=oThis.x;
				oThis.oy=oThis.y;
			}
			//console.log("-----------------onDragend("+wfcanvasId+","+oThis.x+","+oThis.y+","+oThis.ox+","+oThis.oy+")");
			var isnode = (oThis instanceof draw2d.Start) || (oThis instanceof draw2d.UserTask);
			isnode = isnode || (oThis instanceof draw2d.ParallelGateway);
			isnode = isnode || (oThis instanceof draw2d.CallActivity);
			isnode = isnode || (oThis instanceof draw2d.ExclusiveGateway);
			//isnode = isnode || (oThis instanceof draw2d.SubProcess);
			isnode = isnode || (oThis instanceof draw2d.ManualTask);
			isnode = isnode || (oThis instanceof draw2d.End);
			if(isnode){
				if(oThis.ox!=oThis.x || oThis.oy!=oThis.y){
					window.parent.parseClick(wfcanvasId,true,"setCoordinate","{\"wfnodeid\":\""+oThis.getNodeId()+"\",\"x\":\""+oThis.x+"\",\"y\":\""+oThis.y+"\"}");
					oThis.ox=oThis.x;
					oThis.oy=oThis.y;
				}
			}else{
				//console.log("setActionPort()");
			}
		};
...
draw2d.OutputPort.prototype.onDragstart = function(x, y) {
	...
	if(node instanceof draw2d.UserTask || node instanceof draw2d.ParallelGateway){
		if(node.lineCount == 1){
			var red = new draw2d.Color(255,0,0);
			node.workflow.connectionLine.setColor(red);
		}
		if(node.lineCount>=2){
			return false;
		}
...
draw2d.Line.prototype.setPropDailogId = function(propDialogId) {
	this.propDialogId = propDialogId;
	if (this.html !== null) {
		this.html.propDialogId = propDialogId;
	}
};
draw2d.Line.prototype.getPropDailogId = function() {
	return this.propDialogId;
};
draw2d.Line.prototype.setActionId = function(actionId) {
	this.actionId = actionId;
	if (this.html !== null) {
		this.html.actionId = actionId;
	}
};
draw2d.Line.prototype.getActionId = function() {
...
draw2d.Connection = function() {
	...
	this.setLineWidth(1);
	this.initflag = false;
	if(this.type=="DecoratedConnection"){
		this.dcid = this.getId();
	}
};
...
draw2d.Connection.prototype.disconnect = function() {
	if (this.sourcePort !== null) {
		...
		var sourceNode = this.sourceAnchor.getOwner().parentNode;
		sourceNode.lineCount = sourceNode.lineCount - 1;
	}
...
draw2d.Connection.prototype.setTarget = function(port) {
	...
	var sourceNode = this.sourceAnchor.getOwner().parentNode;
	var targetNode = this.targetAnchor.getOwner().parentNode;
	var spnum = this.getPortNum(sourceNode,this.sourcePort.getName());
	var tpnum = this.getPortNum(targetNode,this.targetPort.getName());
	if(!this.initflag){
		//console.log(targetNode.getNodeId());
		//console.log("---------"+this.dcid);
		if(sourceNode.type=="draw2d.UserTask" || sourceNode.type=="draw2d.ParallelGateway"){
			if(sourceNode.lineCount<2){
				//console.log(sourceNode.type);
				if(this.getActionId()){
					window.parent.parseClick(wfcanvasId,true,"setActionPort","{\"actionid\":\""+this.getActionId()+"\",\"snodeid\":\""+sourceNode.getNodeId()+"\",\"tnodeid\":\""+targetNode.getNodeId()+"\",\"sport\":\""+spnum+"\",\"tport\":\""+tpnum+"\"}");
				}else{
					sourceNode.lineCount = sourceNode.lineCount + 1;
					window.parent.parseClick(wfcanvasId,true,"addAction","{\"lineid\":\""+this.getId()+"\",\"snodeid\":\""+sourceNode.getNodeId()+"\",\"tnodeid\":\""+targetNode.getNodeId()+"\",\"sport\":\""+spnum+"\",\"tport\":\""+tpnum+"\"}");
				}
				if(sourceNode.lineCount==2){
					var red = new draw2d.Color(255,0,0);
					var decorator = this.getTargetDecorator();
					decorator.setBackgroundColor(red);
...
draw2d.Connection.prototype.getPortNum = function(node,portname) {
...
draw2d.Connection.prototype.setInitFlag = function(flag) {
	this.initflag = flag;
}
...
draw2d.Workflow = function(id) {
	try {
		...
		wfcanvasId = id;
		wfcanvasReadonly = jq('#'+id).attr("readonly");
		//console.log("wfcanvasReadonly="+this.wfcanvasReadonly);
		this.menu = null;
		this.gridWidthX = 10;
		this.gridWidthY = 10;
		...
		if (this.html !== null) {
			...
			var _4a4b = function(_4a4c) {
				...
				if (line !== null) {
					line.onDoubleClick();
					//console.log("parseClick_"+wfcanvasId+"_"+line.getId());
					window.parent.parseClick(wfcanvasId,true,"selectAction","{\"lineid\":\""+line.getId()+"\",\"dialogid\":\""+line.getPropDailogId()+"\",\"wfactionid\":\""+line.getActionId()+"\"}");
				}
			};
			if (this.html.addEventListener) {
...
draw2d.Workflow.prototype.removeFigure = function(_4a82) {
	//console.log("-----removeFigure-----"+_4a82.type)
...
draw2d.CommandDelete = function(_553a) {
	...
	if(_553a.type=="DecoratedConnection"){
		var snode = _553a.sourceAnchor.getOwner().parentNode;
		snode.lineCount = snode.lineCount -1;
		window.parent.parseClick(wfcanvasId,true,"removeAction","{\"wfactionid\":\""+_553a.getActionId()+"\"}");
	}else{
		if(_553a.getNodeId()){
			window.parent.parseClick(wfcanvasId,true,"removeNode","{\"wfnodeid\":\""+_553a.getNodeId()+"\"}");
		}
	}
};
